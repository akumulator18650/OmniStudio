...
class AIEngine:
    ...
    def load_model(self, model_id: str, precision: str='fp16', vram_mode: str='high', controlnet_id: str=None):
        ...
        try:
            ...
            print(f'[DEBUG] torch.backends.mps.is_available() = {torch.backends.mps.is_available()}')
            if torch.backends.mps.is_available():
            ...
            is_8bit = precision == '8-bit' and self.device == 'mps'
            is_4bit = precision == '4-bit' and self.device == 'mps'
            ...
            def _load(target_id):
                ...
                if is_flux and self.device == 'mps':
                    from diffusers import BitsAndBytesConfig
    ...
    def generate_image(self, prompt: str, num_inference_steps: int=20, width: int=512, height: int=512, guidance_scale: float=7.0, negative_prompt: str=None, seed: int=-1, callback=None, lora_weight: float=1.0, controlnet_id: str=None, control_image=None, batch_size: int=1):
        ...
        if seed is not None and seed >= 0:
            ...
            generator.seed()
        pipe_kwargs = {'prompt': prompt, 'num_inference_steps': num_inference_steps, 'width': width, 'height': height, 'generator': generator, 'guidance_scale': guidance_scale, 'num_images_per_prompt': batch_size}
    ...
    def generate_video(self, image_path: str, prompt: str, output_path: str, negative_prompt: str=None, num_inference_steps: int=25, seed: int=-1, callback=None):
        ...
        try:
            ...
            traceback.print_exc()
            return None
        image = inpaint_pipe(**pipe_kwargs).images[0]
        return image
    ...
    def decode_latents(self, latents):
        if not self.pipe or not hasattr(self.pipe, 'vae') or self.pipe.vae is None:
            return None
        try:
            scaled_latents = (latents / self.pipe.vae.config.scaling_factor).to(self.pipe.vae.dtype)
    ...
    def run_adetailer(self, img: Image.Image, prompt: str, negative_prompt: str=None, seed: int=-1) -> Image.Image:
        ...
        try:
            ...
            enhanced_img = self.generate_inpaint(prompt=face_prompt, init_image=img, mask_image=mask_pil, num_inference_steps=20, strength=0.4, guidance_scale=7.0, negative_prompt=negative_prompt, seed=seed)
            return enhanced_img
        except Exception as e:
